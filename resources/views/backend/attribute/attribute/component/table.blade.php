        ...
        <tr id="{{ $attribute->id }}">
            ...
            <td>
                <div class="uk-flex uk-flex-middle">
                    <div class="image mr5">
                        <div class="img-cover image-list"><img src="{{ asset($attribute->image) }}" alt="{{ $attribute->name }}"></div>
                    </div>
            ...
            <td class="text-center js-switch-{{ $attribute->id }}">
                <input type="checkbox" class="js-switch status" data-field="publish" data-model="{{ $config['model'] }}" value="{{ $attribute->publish }}" data-modelId="{{ $attribute->id }}" {{ ($attribute->publish == 2) ? 'checked' : '' }} />
            </td>
            <td class="text-center">
                @can('modules', 'attribute.update')
                <a href="{{ route('attribute.edit', $attribute->id) }}" class="btn btn-success"><i class="fa fa-edit"></i></a>
                @endcan
                @can('modules', 'attribute.destroy')
