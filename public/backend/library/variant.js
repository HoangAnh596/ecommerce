    ...
    HT.chooseVariantGroup = () => {
        $(document).on('change', '.choose-attribute', function(){
            ...
            if(attributeCatalogueId != 0){
                ...
                _this.parents('.col-lg-3').siblings('.col-lg-8')
    ...
    HT.productVariant = () => {
        ...
        $('.variant-row').each(function(index, value) {
            ...
            if(dataIndex !== -1) {
                ...
                let qty = (variant.quantity[dataIndex] == null || variant.quantity[dataIndex] === '') ? 0 : variant.quantity[dataIndex]
    
                _this.find('.td-quantity').html(addCommas(qty))
                _this.find('.td-price').html(addCommas(variant.price[dataIndex]))
